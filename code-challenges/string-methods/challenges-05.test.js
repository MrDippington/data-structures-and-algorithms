...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(function(element){
    let search = ' ';
    let firstIndex = element.indexOf(search);
    let secondIndex = element.indexOf(search, (firstIndex + 1));
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((string) => {
    ...
    let sliceArr = splitArr.slice(2);
    let pushArr = sliceArr.join(' ');
    result.push(pushArr);
  });
  return result;
};
...
const stepActions = (recipe) => {
  let result = [];
  recipe.steps.forEach(string => {
    let splitArr = string.split(' ');
    let sliceArr = splitArr.slice(0,1);
    let pushArr = sliceArr.join(' ');
    result.push(pushArr);
  });
  return result;
