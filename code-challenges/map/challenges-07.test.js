...
const forEachTwoToThe = (arr) => {
  let newArr = [];
    arr.forEach(function(bananas) {
        let newNumber = 2 ** bananas;
        newArr.push(newNumber);
    })
    return newArr;
};
...
const mapTwoToThe = (arr) => {
  let newArr = arr.map((value, index) => 2 ** value);
    return newArr;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const charCode = (arr) => {
  return arr.map(string => string.charCodeAt());
};
...
const extractStats = (arr) => {
  return arr.map(bananas => ({
    name: bananas.stat.name,
    total: bananas.effort + bananas.baseStat
  }));
};
